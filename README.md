...
The control at top right is labelled "TUNING" is contains the weight settings to which the gates, generated by the Turing Machine, are applied to form the channel A (and channel B) output.

These weight settings are called 'Tunings' and they are selected from my [Tuning Strategy](https://github.com/m0xpd/TuningStrategyForVoltages) to be those which give an output which is musically useful. 
